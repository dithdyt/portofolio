...
const Button: FC<ButtonProps> = ({ children, secondary, icon, onClick }) => {
  return (
    <button
      onClick={onClick}
      className={`${secondary ? "big-secondary" : "bg-accent"} ${
        secondary ? "hover:bg-hoverSecondary" : "hover:bg-hoverPrimary"
      } trasition-all ease-linear duration-300 py-2.5 px-8 rounded-full text-white text-base sm:text-lg text-bold relative w-full sm:w-fit`}
    >
      {icon ? (
